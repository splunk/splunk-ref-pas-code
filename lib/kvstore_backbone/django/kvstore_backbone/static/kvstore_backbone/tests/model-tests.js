...
describe('Model', function() {
  ...
  describe('with autogenerated key', function() {
    var TestModel = KVStore.Model.extend({
      collectionName: 'test'
    });

    it('returns isNew as true without key', function() {
      var model = new TestModel({ });
      expect(model.isNew()).to.be.true;
    });

    it('returns isNew as false with key', function() {
      var model = new TestModel({ _key: 'x44582602dbbb628d0223e70' });
      expect(model.isNew()).to.be.false;
    });

    it('generates right url with default namespace and unset id', function() {
      var model = new TestModel();
    ...
    it('generates right url with default namespace and specified id', function() {
      var model = new TestModel({ _key: 'x44582602dbbb628d0223e70' });
      expect(model.url()).to.equal(
    ...
    it('generates right url with custom namespace and unset id', function() {
    ...
    it('generates right url with custom namespace and specified id', function() {
      var namespace = { owner: 'admin', app: 'not_this_app' };
  ...
  describe('with custom key', function() {
    var TestModelCustomKey = KVStore.Model.extend({
      collectionName: 'test',

      customKey: true
    });

    it('sets isNew to true by default', function() {
      var model = new TestModelCustomKey({ _key: 'other_custom_key' });
      expect(model.isNew()).to.be.true;
    });

    it('respects isNew option', function() {
      var model = new TestModelCustomKey({ _key: 'other_custom_key' }, { isNew: false });
      expect(model.isNew()).to.be.false;
    });

    it('generates right url with default namespace and unset id and isNew true', function() {
      var model = new TestModelCustomKey();
      expect(model.url()).to.equal(
        splunkdUtils.fullpath('storage/collections/data/test', 
          { owner: 'nobody', app: mvcUtils.getCurrentApp() }));
    });

    it('generates right url with default namespace and specified id and isNew true', function() {
      var model = new TestModelCustomKey({ _key: 'x44582602dbbb628d0223e70' });
      expect(model.url()).to.equal(
        splunkdUtils.fullpath('storage/collections/data/test', 
          { owner: 'nobody', app: mvcUtils.getCurrentApp() }));
    });

    it('generates right url with default namespace and specified id and isNew false', function() {
      var model = new TestModelCustomKey({ _key: 'x44582602dbbb628d0223e70' }, { isNew: false });
      expect(model.url()).to.equal(
        splunkdUtils.fullpath('storage/collections/data/test/x44582602dbbb628d0223e70', 
          { owner: 'nobody', app: mvcUtils.getCurrentApp() }));
    });

    it('generates right url with custom namespace and unset id and isNew true', function() {
      var namespace = { owner: 'admin', app: 'not_this_app' };
      var model = new TestModelCustomKey({}, { namespace: namespace });
      expect(model.url()).to.equal(
        splunkdUtils.fullpath('storage/collections/data/test', namespace));
    });

    it('generates right url with custom namespace and specified id and isNew false', function() {
      var namespace = { owner: 'admin', app: 'not_this_app' };
      var model = new TestModelCustomKey({ _key: 'x44582602dbbb628d0223e70' }, { isNew: false, namespace: namespace });
      expect(model.url()).to.equal(
        splunkdUtils.fullpath('storage/collections/data/test/x44582602dbbb628d0223e70', namespace));
    });
    ...
    it('does not change options object on save', function(done) {
      var model = new TestModelCustomKey({ _key: 'custom_key' });
      var options = {};
      model.save({}, options)
        .done(function(data, textStatus, jqXHR) {
          expect(options).to.be.empty;
    ...
    it('does not change options object on destroy', function(done) {
      var model = new TestModelCustomKey({ _key: 'custom_key' }, { isNew: false });
      var options = {};
      model.destroy(options)
        .done(function(data, textStatus, jqXHR) {
          done(new Error('Fail expected'));
    ...
    it('does not drop collection with model is new', function(done) {
      var emptyModel = new TestModelCustomKey({ _key: 'new_custom_key' });
      var savedModel = new TestModelCustomKey({ name: 'a', _key: 'custom_key' }, { isNew: true });
      var copyModel = new TestModelCustomKey({ _key: 'custom_key' }, { isNew: false });
    ...
    it('does not allow to save with empty string as a key', function(done) {
      var model = new TestModelCustomKey({ _key: '' });
    ...
    describe('with non default namespace', function() {
      ...
      beforeEach(function(done) {
        cleanCurrentUserCollectionAsync(function() {
          model = new TestModelCustomKey({name: 'test', _key: 'custom_key' }, { namespace: { owner: currentUser } });
          model.save()
            .done(function(data, textStatus, jqXHR) {
              expect(model.id).to.be.a('string');
              expect(model.id).to.be.not.empty;
      ...
      // Verify that inserted model exists on server
      it('can be created', function(done) {
        var modelCopy = new TestModelCustomKey({ _key: 'custom_key' }, { isNew: false, namespace: { owner: currentUser } });
        modelCopy.fetch()
          .done(function(data, textStatus, jqXHR) {
            expect(modelCopy.toJSON()).to.deep.equal(model.toJSON());
            done();
      ...
      it('cannot be read from other namespaces', function(done) {
        var modelUser = new TestModelCustomKey(
          { _key: model.id },
          { isNew: false }
        );
        modelUser.fetch()
    ...
    describe('saved on server', function() {
      ...
      // Insert one model before each test
      beforeEach(function(done) {
        model = new TestModelCustomKey({ name: 'test', _key: customKeyValue });
        model.save()
          .done(function(data, textStatus, jqXHR) {
            expect(model.id).to.be.a('string');
            expect(model.id).to.be.not.empty;
      ...
      // Verify that inserted model exists on server
      it('can be created', function(done) {
        var modelCopy = new TestModelCustomKey({ _key: customKeyValue}, { isNew: false });
        modelCopy.fetch()
          .done(function(data, textStatus, jqXHR) {
            expect(modelCopy.toJSON()).to.deep.equal(model.toJSON());
            done();
      ...
      it('does not allow duplicates', function(done) {
        var modelDuplicate = new TestModelCustomKey({ _key: customKeyValue, name: 'test2' });
        ...
        modelDuplicate.save()
          ...
          .fail(function(jqXHR, textStatus, errorThrown) {
            expect(jqXHR.status).to.equal(409);
            done();
          });
      });
    });
